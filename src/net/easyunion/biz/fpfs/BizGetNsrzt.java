...
public class BizGetNsrzt implements IBiz{
	...
	public String doWrok(Document doc) {
		...
		try {
			...
			if(Const.CTAIS.equals(channel)){
				list = fpfsCtaisService.getNsrzt(djxh);
			}else if(Const.JSSQ_HXZG.equals(channel)){
				list = fpfsJssqService.getNsrzt(djxh);
			}
	...
	private String getResult(List<Map<String, Object>> list){
		...
		if(list!=null){
			for(Map<String, Object> map:list){
				result.append("<getNsrzt>");
				result.append("<nsrzt>").append(map.get("nsrzt_dm")).append("</nsrzt>");
				result.append("</getNsrzt>");
			}
		}
