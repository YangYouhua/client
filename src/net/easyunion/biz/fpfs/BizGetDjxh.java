...
public class BizGetDjxh implements IBiz{
	
	private final static Logger logger = LoggerFactory.getLogger(BizGetDjxh.class);
	...
	public String doWrok(Document doc) {
		...
		try {
			...
			if(Const.CTAIS.equals(channel)){
				list = fpfsCtaisService.getDjxh(djxh);
			}else if(Const.JSSQ_HXZG.equals(channel)){
				list = fpfsJssqService.getDjxh(djxh);
			}
	...
	private String getResult(List<Map<String, Object>> list){
		...
		if(list!=null){
			for(Map<String, Object> map:list){
				result.append("<getDjxh>");
				result.append("<djxh>").append(map.get("djxh")).append("</djxh>");
				result.append("</getDjxh>");
			}
		}
