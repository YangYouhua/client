...
public class BizGetSmz implements IBiz{
	...
	public String doWrok(Document doc) {
		...
		try {
			...
			if(Const.CTAIS.equals(channel)){
				list = fpfsCtaisService.getSmz(nsrsbh);
			}else if(Const.JSSQ_HXZG.equals(channel)){
				list = fpfsJssqService.getSmz(nsrsbh);
			}
	...
	private String getResult(List<Map<String, Object>> list){
		...
		if(list != null && list.size() > 0){
			for(Map<String, Object> map:list){
				result.append("<getSmz>");
				result.append("<fddbrsfzjhm>").append(map.get("fddbrsfzjhm")).append("</fddbrsfzjhm>");
				result.append("</getSmz>");
			}
		}else {
			result.append("<getSmz><fddbrsfzjhm>0</fddbrsfzjhm></getSmz>");
		}
