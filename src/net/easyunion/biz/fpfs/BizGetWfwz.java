...
public class BizGetWfwz implements IBiz{
	...
	public String doWrok(Document doc) {
		...
		try {
			...
			if(Const.CTAIS.equals(channel)){
				list = fpfsCtaisService.getWfwz(nsrsbh);
			}else if(Const.JSSQ_HXZG.equals(channel)){
				list = fpfsJssqService.getWfwz(nsrsbh);
			}
	...
	private String getResult(List<Map<String, Object>> list){
		...
		if(list != null && list.size() > 0){
			for(Map<String, Object> map:list){
				result.append("<getWfwz>");
				result.append("<sswfxwclzt_dm>").append(map.get("sswfxwclzt_dm")).append("</sswfxwclzt_dm>");
				result.append("</getWfwz>");
			}
		}else {
			result.append("<getWfwz><sswfxwclzt_dm>0</sswfxwclzt_dm></getWfwz>");
		}
